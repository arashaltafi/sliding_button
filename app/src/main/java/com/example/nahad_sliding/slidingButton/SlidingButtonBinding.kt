...
import com.example.nahad_sliding.databinding.LayoutSlideButtonBinding
import kotlin.math.max

class SlidingButtonBinding : FrameLayout {

    private lateinit var binding: LayoutSlideButtonBinding
    ...
    val currentTextColor = if (::slidingText.isInitialized) slidingText.currentTextColor else textColors?.defaultColor

    val currentTextColorLogin = if (::slidingTextLogin.isInitialized) slidingTextLogin.currentTextColor else textColorsLogin?.defaultColor

    val currentTextColorSwipe = if (::slidingTextSwipe.isInitialized) slidingTextSwipe.currentTextColor else textColorsSwipe?.defaultColor
    ...
    private fun configureTrackView() {
        ...
        binding.apply {
            ...
            if (showIndicator) {
                ...
                slidingIndicator.viewTreeObserver.addOnGlobalLayoutListener {
                    ...
                    when (trackExtendedTo) {
                        ...
                        TrackExtended.CONTAINER -> {
                            lp.width = 0
                            lp.height = this@SlidingButtonBinding.measuredHeight
                            lp.marginStart = 0
                            lp.marginEnd = 0
                        }
                    }
...
//                        inflatedView.viewTreeObserver.removeOnGlobalLayoutListener(this)
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        endOfButton = w.toFloat() - (buttonWidth.toFloat() + buttonMargins[2].toFloat() + paddingEnd.toFloat() + paddingStart.toFloat())
    }
    ...
    override fun removeViewAt(index: Int) = throw IllegalStateException("This method isn't allowed ")
    ...
    fun setGravity(isRtl: Boolean) {
        binding.apply {
            if (isRtl) {
                slidingImage.gravity = Gravity.START or Gravity.CENTER
            } else {
                slidingImage.gravity = Gravity.END or Gravity.CENTER
            }
        }
    }
